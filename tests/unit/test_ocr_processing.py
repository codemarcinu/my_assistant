...
class TestOCRProcessing:
    ...
    def test_extract_text_from_image_obj(self, mock_image):
        ...
        with patch(
            "pytesseract.image_to_string", return_value=expected_text
    ...
    def test_process_image_file(self):
        ...
        with patch("PIL.Image.open") as mock_open, patch(
            "backend.core.ocr._extract_text_from_image_obj", return_value=expected_text
        ) as mock_extract:
    ...
    def test_process_pdf_file(self):
        ...
        with patch("tempfile.NamedTemporaryFile") as mock_temp, patch(
            "fitz.open", return_value=mock_pdf
        ) as mock_open, patch("PIL.Image.frombytes") as mock_frombytes, patch(
            "backend.core.ocr._extract_text_from_image_obj", return_value=page_text
        ) as mock_extract:
    ...
    def test_ocr_processor_process_image(self, mock_tesseract_data):
        ...
        with patch("PIL.Image.open") as mock_open, patch(
            "pytesseract.image_to_data", return_value=mock_tesseract_data
            ...
            expected_confidence = sum(mock_tesseract_data["conf"]) / len(
                mock_tesseract_data["conf"]
            )
    ...
    def test_ocr_processor_process_pdf(self):
        ...
        mock_result = OCRResult(
            text="Page content", confidence=90.0, metadata={"source": "image"}
        )
    ...
    def test_ocr_processor_process_pdf_error_handling(self):
        ...
        with patch(
            "tempfile.NamedTemporaryFile", side_effect=Exception("Mock PDF error")
        ):
            result = processor.process_pdf(sample_bytes)
    ...
    def test_ocr_processor_process_images_batch(self):
        ...
        mock_result1 = OCRResult(
            text="Image 1 content", confidence=91.0, metadata={"source": "image"}
        )
        mock_result2 = OCRResult(
            text="Image 2 content", confidence=88.5, metadata={"source": "image"}
        )

        with patch.object(
            processor, "process_image", side_effect=[mock_result1, mock_result2]
        ) as mock_process:
